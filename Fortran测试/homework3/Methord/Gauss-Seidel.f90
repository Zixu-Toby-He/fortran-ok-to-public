!本子程序用于利用高斯-赛德尔迭代法进行方程的求根
!该方法迭代时，x(i)每算出一个新值，都要将它作为旧值参与x(i+1)的迭代
!而所利用的矩阵由于对角项均为0，故在循环时可以利用x_new(i)=ΣG(i,j)*x_new(j)+b(j)来赋值实现
!个人还做了其对应的矩阵方法，经检验其可以发挥作用，但实际效果和直接计算仍然有差距
